...
struct VIEW_RESEARCH : VIEW_BASE
{
	...
	std::string		sequenceName;	// which windowed flic to display
	std::string		audio;			// name of audio track to play (for this seq)
};
...
struct SEQ_DISPLAY
{
	...
	std::vector<std::string>	textMsg;	//Text messages - if any
	std::string				audio;		// name of audio track to play (for this seq)
};
...
struct VIEWDATA
{
	std::string				name;			//name ID of the message - used for loading in and identifying
	...
	std::string				fileName;       // file it came from, for piecemeal destruction (pretty lame reason)
};
