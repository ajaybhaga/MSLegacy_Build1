...
{
	SIMPLE_OBJECT(OBJECT_TYPE type, uint32_t id, unsigned player, float gameTime);
	virtual ~SIMPLE_OBJECT();

	const OBJECT_TYPE type;                         ///< The type of object
	uint32_t        id;                             ///< ID number of the object
	IntVector3        pos = IntVector3(0, 0, 0);        ///< IntVector3 of the object
	Quaternion        rot;                            ///< Object's yaw +ve rotation around up-axis
	...
	uint32_t        time;                           ///< Game time of given space-time IntVector3.
};
...
{
	Spacetime() {}
	Spacetime(IntVector3 pos_, Quaternion rot_, uint32_t time_) : time(time_), pos(pos_), rot(rot_) {}

	uint32_t time = 0;                  ///< Game time
	IntVector3 pos = IntVector3(0, 0, 0);   ///< IntVector3 of the object
	Quaternion rot;                       ///< Quaternion of the object
};
...
static inline int objPosDiffSq(Vector3 pos1, Vector3 pos2)
{
	const Vector3 diff = (pos1 - pos2);
	return diff.DotProduct(diff);
}

static inline int objPosDiffSq(SIMPLE_OBJECT const *pos1, SIMPLE_OBJECT const *pos2)
{
	return objPosDiffSq(Vector3(pos1->pos.x_, pos1->pos.y_, pos1->pos.z_), Vector3(pos2->pos.x_, pos2->pos.y_, pos2->pos.z_));
}
